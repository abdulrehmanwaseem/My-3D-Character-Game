...
export function CharacterModel({ animation = false, ...props }) {
  ...
  useEffect(() => {
    if (actions) {
      const action = actions["Armature|Take 001|BaseLayer"]; // Adjust the action name as necessary
      if (animation && action) {
        action.play(); // Play the animation if the `animation` prop is true
      } else if (action) {
        action.stop(); // Stop the animation if the `animation` prop is false
      }
    }
